...
Within each of these subdirectories, copy and paste the *contents* of the `BST` folder. You don't need to copy and paste the `HELP` folder every time, but you do need to copy and paste the other directories: `HWIR`, `RISK_RESULTS`, `SOURCE_RESULTS`, `VVWM`. These directories contain files that the Microsoft Access implementation of the BST needs in order to run. 
...
The MS Access implementation of the BST downloaded from the URL above does not include bulk upload capabilities. To run this analysis, you will need to get the version with bulk upload capabilities. 

To do this, download the Supplemental Material associated with the manuscript  "A Cheminformatics Workflow for Higher-throughput Modeling of Chemical Exposures from Biosolids" (Kruse et al., 2025). There is a file in that Supplemental Material named `BST_BulkUpload.mdb`. This is the version of the BST with bulk upload capabilities.
...
Rename the exported Excel workbook files. for the NSSS mean runs for the LAU scenarios, you might want to name the file "BST_results_LAU_mean.xlsx."
