---
title: "autoBST"
author: "Paul Kruse and Caroline Ring"
date: "2025-03-24"
...
Ensure your working directory in R is the directory where this R Markdown document lives (using commands `getwd()` to check the current working directory, and `setwd()` to change it if needed).

Load necessary packages. All are available on CRAN except for `ggsankey`.
...
Load in the template sheets from the Bulk Uploader Tool: templates for organic chemicals, inorganic chemicals, and ecological data associated with each chemical.

If you open the template file `data/BST_BulkChemicalImportTool.xlsm` in Microsoft Excel, you will find a good deal of information and context about the various sheets in the template. You will also find instructions to use the template file by manually inputting chemical information into the spreadsheet and then clicking buttons to run certain macros. You can ignore these instructions if you are using autoBST. autoBST reproduces what would be contained in the file after manual input of data and running all macros; you won't need to do anything. autoBST only uses `data/BST_BulkChemicalImportTool.xlsm` as a template to format its output. 
...
biosolids_curated <- biosolids_curated[2:nrow(biosolids_curated), ]

#also read in nutrients
biosolids_curated2 <- readxl::read_excel("data/Richman_et_al_2022_SI_Table1_Master List of Chemicals Detected in Biosolids.xlsx",
                                        "Nutrients Detected in Biosolids",
                                        "A1:D8")
#remove extra blank row
biosolids_curated2 <- biosolids_curated2[2:nrow(biosolids_curated2), ]

biosolids_curated <- data.table::rbindlist(list(biosolids_curated,
                                                biosolids_curated2),
                                           use.names = FALSE)

chem_details <- ctxR::get_chemical_details_batch(DTXSID = biosolids_curated$DTXSID)
```
...
Because querying the ClassyFire API can be slow, we highly recommend only running this section once, and *saving* the result to re-load later. The ClassyFire API is limited to a maximum of 12 requests per minute; with 727 unique InCHiKeys in the biosolids curated list, that translates to approximately 61 minutes. The results of the query have been pre-saved in the `data/` folder.

```{r, eval = FALSE}
system.time(
  chem_classes <- classify_inchikeys(inchikeys = chem_details[!is.na(inchikey),
                                                      unique(inchikey)])
)
...
saveRDS(chem_classes,
        "data/biosolids_curated_ClassyFire.Rds")
```

Read in the saved ClassyFire classifications.

```{r}
chem_classes <- readRDS("data/biosolids_curated_ClassyFire.Rds")
```
...
```{r}
previously_loaded_casrns <- data.table::fread("data/BST previously loaded CASRNs.csv")
```
...
generate_organic_entry <- function(DTXSID,
  ...
  chemical_details <- ctxR::get_chemical_details(DTXSID = DTXSID)
    ...
    chemical_details <- list(casrn = NA_character_,
                             preferredName = NA_character_,
                             monoisotopicMass = NA_real_)
  }
  
  # Get chemical info
  chemical_info <- data.table(ctxR::get_chem_info(DTXSID = DTXSID))
  
    if(length(chemical_info) == 0){
     chemical_info <-  as.data.table(
     expand.grid(name = c('Molar Volume',
                                         "Henry's Law",
                                         'KOC',
                                         'LogKow: Octanol-Water'
                                         ),
                                propType = c('predicted', 'experimental'),
                                value = NA_real_,
  ...
  if (is.nan(chemical_koc)){
    if (!is.na(chemical_logkow)){
      chemical_koc <- calc_koc(DTXSID = DTXSID,
                               pH = 7,
...
make_sankey <- function(filt_dt,
  ...
  filt_dt <-  filt_dt %>% 
  #for readability, in case there is a long list of values missing or out of range --
#drop the derived ones (Br[...], Bv, BCF_beef, BCF_milk)
  mutate(missing_data = gsub(x = missing_data,
                             pattern = "B[rv]\\w*(,\\s)?",
                             replacement = "",
                             perl = TRUE)) %>%
  mutate(missing_data = gsub(x = missing_data,
                             pattern = "BCF_((beef)|(milk))(,\\s)?",
                             replacement = "",
                             perl = TRUE)) %>%
    mutate(missing_data = gsub(x = missing_data,
                             pattern = "RCF(,\\s)?",
    ...
    mutate(missing_data = gsub(x = missing_data,
                               pattern = "LogKow",
                               replacement = "Kow")) %>%
    mutate(missing_data =gsub(
                                               x = missing_data,
                                               pattern = "Da, Dw",
                                               replacement = "MV",
                                               fixed = TRUE
                                             )) %>%
    #if it's more than 3 items, split it onto two lines
    mutate(missing_data = if_else(sapply(strsplit(missing_data,
                                                  split = ", "),
                                         length) > 3,
                                 
                                           true = sapply(strsplit(missing_data,
                                                  split = ", "),
                                                  function(this_tmp){
                                                paste0(
                                                  paste(this_tmp[1:3],
                                                      collapse = ", "),
                                                    "\n",
                                                    paste(this_tmp[4:(length(this_tmp))],
                                                          collapse = ", ")
                                                )
                                                  }
                                                ),
  ...
  mutate(missing_data = factor(missing_data,
                                      levels = c("preloaded",
                                      "no missing",
                                                 setdiff(unique(missing_data),
                                                         c("no missing",
...
ggsave( "results/BST_sankey_1ppb.pdf",
        my_plot1ppb,
       height = 7,
       width = 9,
       units = "in")

ggsave( "results/BST_sankey_1ppb.png",
        my_plot1ppb,
       height = 7,
       width = 9,
       units = "in",
...
ggsave("results/BST_sankey_NSSSmean.pdf",
       my_plot_mean, 
       height = 5,
       width = 9,
       units = "in")

ggsave("results/BST_sankey_NSSSmean.png",
       my_plot_mean, 
       height = 5,
       width = 9,
       units = "in")
...
ggsave("results/BST_sankey_NSSS95th.pdf",
       my_plot_95, 
       height = 5,
       width = 9,
       units = "in")

ggsave("results/BST_sankey_NSSS95th.png",
       my_plot_95, 
       height = 5,
       width = 9,
       units = "in")
...
Download the Biosolids Screening Tool from [https://www.epa.gov/biosolids/risk-assessment-pollutants-sewage-sludge](https://www.epa.gov/biosolids/risk-assessment-pollutants-sewage-sludge). (Direct link: [https://www.epa.gov/system/files/other-files/2023-03/biosolids-screening-tool.zip](https://www.epa.gov/system/files/other-files/2023-03/biosolids-screening-tool.zip).)

Before unzipping the BST, create a directory with as short a pathname as possible. The BST must be run in a directory with a pathname of 48 characters or less, according to the BST User Guide. This probably means you need to create the directory close to the root of `C:\`. For example, Dr. Kruse created the directory at `C:\Users\pkruse\BST`.

Unzip the BST download into your short-pathname directory. A new folder called "BST" will appear.

Within the short-pathname directory, create one subdirectory for each set of input concentrations: 1 ppb; NSSS mean; and NSSS 95th percentile. Name them (for example) `ppb`, `mean`, and `p95`, respectively, to keep track of which is which.

Within each of these subdirectories, copy and paste the *contents* of the `BST` folder. You don't need to copy and paste the `HELP` folder every time, but you do need to copy and paste the other directories: `HWIR`, `RISK_RESULTS`, `SOURCE_RESULTS`, `VVWM`. These directories contain helper files that the Microsoft Access implementation of the BST needs in order to run. 

The MS Access implementation of the BST itself lives in the `.mdb` file. You do not need to copy and paste the `.mdb` file; it will be replaced in the next step anyway.

## Set up the version of the BST with bulk upload capabilities

The MS Access implementation of the BST downloaded from the URL above does not include bulk upload capabilities. The file `BST_BulkUpload.mdb`, in the same directory as this R Markdown file, is the version of the BST with bulk upload capabilities. 

Now, within each subdirectory (`mean`, `p95`, and `ppb`), make a new copy of the file `BST_BulkUpload.mdb`.

Optionally, you may want to rename these files to make it clear which subdirectory they are associated with, e.g., as `BST_BulkUpload_mean.mdb`, `BST_BulkUpload_p95.mdb`, and `BST_BulkUpload_ppb.mdb`.

## Place the Bulk Upload worksheet (output of autoBST)

Now, copy and paste the Bulk Upload workbooks generated by the autoBST into their corresponding folders. By default, the above code will have saved them in the `results/` subdirectory that should be alongside this R Markdown document in the current R working directory. Copy and paste `results/curated1.xlsx` into `ppb`; `results/NSSSmean.xlsx` into `mean`; and `results/NSSS95th.xlsx` into `p95`.
...
Now, choose a subdirectory to start with: for example, the `mean` subdirectory. Double-click on the `.mdb` file you just created to open it in Microsoft Access.
...
Also, make sure you are looking at the version of the BST that includes bulk-upload capabilities: **without clicking any buttons**, look at the column of buttons on the left and check to make sure that you *see* a button labeled "Bulk Import Chemicals" (**do not click it yet**).

### Manually edit the hazard/toxicity values for the pre-loaded chemicals 

Manually edit the hazard/toxicity values for the pre-loaded chemicals: set CSForal, IUR, RfC, and RfD equal to 1 for all pre-loaded chemicals. This makes it so that all the "risk" and "hazard quotient" values output by the BST actually represent exposure doses or concentrations. This is the only way to get the BST to output average daily dose that is the total of all ingestion pathways or inhalation pathways. 

If you want the original pre-loaded values for CSForal, IUR, RfC, and RfD, they are still populated in the original file `BST_BulkUpload.mdb`.

### Edit pre-loaded biosolids concentrations

In the subdirectory `ppb`, for simulations with 1 ppb biosolids concentrations, you will need to edit the preloaded chemicals so that `CTPWasteDry` takes a value of $1 \times 10^{-3}$ mg/kg (equivalent to 1 ppb).

When running simulations with mean NSSS biosolids concentrations, you will need to edit the preloaded chemicals so that `CTPWasteDry` reflects the mean NSSS concentration.

When running simulations with 95th percentile NSSS biosolids concentrations, you do *not* need to edit the preloaded chemicals. For the preloaded chemicals, `CTPWasteDry` by default reflects the 95th percentile NSSS concentration, except for a few chemicals specified in the next section.
...
These chemicals should be deselected when you are running simulations with mean or 95th percentile biosolids concentrations.
...
We highly recommend keeping a "clean" version of the BST file as it was just before you performed the bulk import, including any edits that you have made manually to the pre-loaded chemicals. This is because **you cannot delete or overwrite data for any chemicals once they are imported**. If you realize that you need to change something in the autoBST and re-generate new Bulk Upload worksheets with new data, then it will be much easier if you can simply make a new copy of the file and re-do the bulk import with the new workshets.

Next, create a copy of the "clean" file, and rename it. You can use whatever name you like, as long as it fits into the 48-character path name length limit.

Open this newly-renamed Access file. This is where you will perform the bulk upload.
...
Click "View Results", click "LAU Results," and export the results to Excel.

Rename the exported Excel workbook files. For example, for the NSSS mean runs for the LAU scenarios, you might want to name the file "BST_results_LAU_mean.xlsx."

## Moving the BST run results for analysis

Copy and paste the exported BST result spreadsheets into the `results/` subdirectory that should be in the same directory alongside this R Markdown file.

That directory already contains the exported BST result spreadsheets from the BST runs performed for "A Cheminformatics Workflow for Higher-throughput Modeling of Chemical Exposures from Biosolids" (Kruse et al. 2025). The output file for the NSSS mean concentration scenario is in `results/BST_results_LAU_mean.xlsx`; the output file for the NSSS 95th percentile concentration scenario is in `results/BST_results_LAU_mean_p95.xlsx`; and the output file for the 1 ppb concentration scenario is in `results/BST_results_LAU_mean_ppb.xlsx`.

The following code refers to those pre-existing output files.

# Analysis of BST results

Make an `analysis/` directory to hold the results of the analysis, if it does not already exist.
...
Ensure any chemicals that should have been removed from the NSSS scenarios are removed.

```{r}
df <- df %>%
  filter(!(which_conc %in% c("mean", "p95") &
             `Chemical Name` %in% c(
               "Chromium (III)",
               "Chromium (VI)",
               "Mercury (divalent)",
...
Plot frequency polygons (like histograms) for the adult farmer ADDs, under the three LAU scenarios and the three biosolids concentration scenarios.
...
ggsave(paste0("analysis/BST_ADD_freqpoly.pdf"),
        width = 9.75,
       height = 5.4)
ggsave(paste0("analysis/BST_ADD_freqpoly.png"),
       width = 9.75,
       height = 5.4,
       units = "in",
       dpi = 300)
...
```{r}
df %>%
  group_by(which_conc, Scenario) %>%
  summarise(min_ADD = min(Value, na.rm = TRUE),
            max_ADD = max(Value, na.rm = TRUE))
```


Compare biosolids concentrations to adult farmer ADDs for each LAU scenario and each biosolids concentration scenario (excluding 1 ppb, since concentration is constant for that scenario). Make plots and run linear regression (on a log10-log10 scale).

```{r}
#read in mapping between DTXSID and stripped CASRN
#pull this from the 1 ppb spreadsheet
mapping <- readxl::read_excel("results/curated1ppb.xlsx",
                   ...
                   function(which_conc){
                     ...
                     if(which_conc %in% "mean"){
                       ...
                       this_tbl <- make_conc_tbl(which_conc = "95th") %>%
                         ...
                         dplyr::rename(CAS = casrn_stripped) %>%
                         dplyr::mutate(CAS = as.numeric(CAS))
                     }else{ #if which_conc == "ppb"
                       this_tbl <- mapping %>%
                         dplyr::mutate(conc_mgkg = 1e-3) %>%
                         dplyr::rename(CAS = casrn_stripped)
                     }
...
conc_tbl <- conc_tbl %>%
  ...
  dplyr::mutate(conc_rank = 1:dplyr::n()) %>%
  dplyr::ungroup()
...
df_conc <- df %>%
  ...
  dplyr::mutate(conc_mgkg = dplyr::if_else(which_conc %in% "ppb" &
                                             is.na(conc_mgkg),
                                           0.001,
                                           conc_mgkg)
                )
...
write.csv(df_lm,
          "analysis/Rsquared_logADD_logconc.csv")
df_lm <- df_lm %>%
...
ggsave("analysis/ADD_vs_conc.pdf",
       width = 9.75,
       height = 5.4,
       units = "in")
ggsave("analysis/ADD_vs_conc.png",
        width = 9.75,
       height = 5.4,
       units = "in",
       dpi = 300)
...
Add Log Kow to the regression of adult farmer ADD on concentration (by LAU scenario and by biosolids concentration scenario).

```{r}

#plot ADD vs. concentration gray-scaled by log Kow
#get log Kow information
#this will be pulled from the mean results workbook,
#but it doesn't matter which results workbook we use
this_input <- readxl::read_excel(
  "results/BST_results_LAU_mean.XLSX",
 ...
 df_conc2 <- df_conc %>%
   dplyr::left_join(this_input,
                    by = "CAS") %>%
   dplyr::mutate(LogKow = as.numeric(LogKow))
...
ggsave("analysis/ADD_vs_conc_logkow.pdf",
        width = 9.75,
       height = 5.4)
ggsave("analysis/ADD_vs_conc_logkow.png",
       width = 9.75,
       height = 5.4,
       units = "in",
       dpi = 300)
...
write.csv(df_lm2,
          "analysis/Rsquared_logADD_logconc_logKow.csv")
```

Get rankings of chemicals by adult farmer ADD within LAU scenario and within biosolids concentration scenario.
...
df_rank <- df_conc %>%
  ...
  ungroup() %>%
  rename(ADD = Value) %>%
  select(c(DTXSID, CAS, `Chemical Name`, which_conc, conc_mgkg, conc_rank, Scenario, ADD, Rank)) %>%
  rename(Adult_Farmer_ADD = ADD) %>%
 tidyr::pivot_wider(id_cols = c(DTXSID, CAS, which_conc, `Chemical Name`, conc_mgkg, conc_rank),
              names_from = Scenario,
              values_from = c(Adult_Farmer_ADD,
                              Rank)) %>%
  dplyr::mutate(conc_rank = dplyr::if_else(which_conc %in% "ppb",
                                           NA_real_,
                                           conc_rank))

  write.csv(df_rank, "analysis/ADD_rank.csv")
```
