...
make_conc_tbl <- function(which_conc = "mean" #or "95th"
  ...
  DT_long <- melt(NSSS,
                  id.vars = "DTXSID",
                  measure.vars = list("conc" = vars,
                                      "unit" = units_vars),
                  na.rm = TRUE,
                  variable.factor = FALSE)
  
  DT_long[, conc_variable := vars[as.integer(variable)]]
...
## Exposures from mean biosolids concentrations

Read in the BST results from the land-application scenario.

```{r}
mean_out <- readxl::read_excel("data/BST_results_LAU_mean.XLSX",
                               sheet = "Human_Results")
```

The variable `Value`, for `Health Benchmark == 'Noncancer'` and `Pathway == 'Total Ingestion'`, reflects the Average Daily Dose (ADD) in mg/kg/day. (Usually, this `Value` would reflect the Hazard Quotient or HQ, which is simply the ADD divided by the reference dose (RFD). However, the parameter "RFD" was set equal to 1. This makes the BST output the ADD, instead of the HQ.)

Filter the output data to include only `Receptor %in% 'Adult Farmer'`, `Health Benchmark == 'Noncancer'`, and `Pathway == 'Total Ingestion'`. This focuses the analysis on ADD for adult human receptors.
...
Predicted average daily doses from ingestion pathways span a wide range of values.
...
ggplot(mean_add) +
  ...
  ggtitle("NSSS mean concentrations") +
  theme_bw() +
...
Plot chemicals ranked by ADD. Order the chemicals from lowest to highest ADD for the Crop scenario (this is just an arbitrary choice of scenario).

```{r}
mean_add_adult_crop <- mean_add %>%
  dplyr::filter(Receptor %in% 'Adult Farmer' &
                  Scenario %in% 'Crop') %>%
  dplyr::arrange(Value) %>%
  dplyr::mutate(Chemical_fct = factor(`Chemical Name`,
                                      levels = unique(`Chemical Name`)))

ggplot(mean_add_adult) +
  geom_point(aes(x = `Chemical Name`,
                 y = Value,
                 color = Scenario)) +
  scale_x_discrete(limits = mean_add_adult_crop %>% pull(Chemical_fct)) +
  scale_y_log10(guide = "axis_logticks") +
  ylab("ADD, mg/kg/day") +
  xlab("Chemical") +
    ggtitle("BST ADDs for NSSS mean conc., LAU scenarios") +
  theme_minimal() +
...
### Comparing LAU scenarios

Are exposures consistently higher for one of the LAU scenarios?

To answer this question, produce a scatterplot matrix, plotting ADD for each LAU scenario vs. ADD for each of the other LAU scenarios.

```{r}
#pivot the data wider so that there are separate columns for ADD for each LAU scenario
mean_add_wide <- mean_add %>% pivot_wider(id_cols = c(Climate, CAS,
                                     `Chemical Name`,
                                     Receptor,
                                     Pathway,
                                     `Health Benchmark`),
...
#Utility function to add an identity line to a `pairs` plot
...
pairs(formula = ~ Crop + Pasture + Reclamation,
      data = mean_add_wide,
      upper.panel = my_line,
      lower.panel = NULL,
      log = "xy",
      main = "Adult ADDs for NSSS mean conc.")
```

None of the scenarios yield consistently higher or lower exposures than the others.

### Rankings from high to low ADD for each LAU scenario

```{r}
rank_crop <- mean_add_adult_crop %>%
  arrange(-Value) %>%
dplyr::select(`Chemical Name`, Value) %>%
  dplyr::rename(Crop = Value) %>%
  dplyr::mutate(Crop_Rank = 1:nrow(mean_add_adult_crop))

rank_rec <- mean_add_adult_rec %>%
  arrange(-Value) %>%
dplyr::select(`Chemical Name`, Value) %>%
  dplyr::rename(Reclamation = Value)  %>%
  dplyr::mutate(Reclamation_Rank = 1:nrow(mean_add_adult_rec))

rank_pasture <- mean_add_adult_pasture %>%
  arrange(-Value) %>%
dplyr::select(`Chemical Name`, Value) %>%
  dplyr::rename(Pasture = Value)  %>%
  dplyr::mutate(Pasture_Rank = 1:1:nrow(mean_add_adult_pasture))

rank_all <- rank_crop %>%
  full_join(rank_pasture, by = join_by(`Chemical Name`)) %>%
  full_join(rank_rec, by = join_by(`Chemical Name`))
```

Produce a table including both rank and dose info

```{r}
rank_mean <- rank_all %>%
   dplyr::rename(Crop_ADD = Crop,
                 Pasture_ADD = Pasture,
                 Reclamation_ADD = Reclamation) %>%
  dplyr::mutate(Crop = paste0(sprintf("%.3g",
                                      Crop_ADD),
                              "(",
                              Crop_Rank,
                              ")"),
                Pasture = paste0(sprintf("%.3g",
                                      Pasture_ADD),
                              "(",
                              Pasture_Rank,
                              ")"),
                Reclamation = paste0(sprintf("%.3g",
                                      Reclamation_ADD),
                              "(",
                              Reclamation_Rank,
                              ")")) %>%
  write.csv("data/ADD_rank_mean.csv")
```

Display the chemicals that were in the top 20 for each LAU scenario.
...
## Exposures from 95th percentile biosolids concentrations

```{r}
p95_out <- readxl::read_excel("data/BST_results_LAU_p95.XLSX",
                               sheet = "Human_Results")
```

The variable `Value`, for `Health Benchmark == 'Noncancer'` and `Pathway == 'Total Ingestion'`, reflects the Average Daily Dose (ADD) in mg/kg/day. (Usually, this `Value` would reflect the Hazard Quotient or HQ, which is simply the ADD divided by the reference dose (RFD). However, the parameter "RFD" was set equal to 1. This makes the BST output the ADD.)

Filter the output data to include only `Receptor %in% 'Adult Farmer'`, `Health Benchmark == 'Noncancer'`, and `Pathway == 'Total Ingestion'`.
...
ggplot(p95_add) +
  ...
  scale_color_brewer(palette = "Set2") +
  xlab("BST-predicted total ingestion ADD (adult farmer), mg/kg/day") +
  ggtitle("Exposures from NSSS 95th pctile biosolids concentrations") +
  theme_bw() +
  theme(legend.position = c(0.8,0.8),
        legend.text = element_text(size = 12),
        legend.title = element_blank(),
...
ggplot(p95_add_adult) +
  geom_point(aes(x = `Chemical Name`,
                 y = Value,
                 color = Scenario)) +
  scale_x_discrete(limits = p95_add_adult_crop %>% pull(Chemical_fct)) +
  scale_y_log10(guide = "axis_logticks") +
  ylab("ADD, mg/kg/day") +
  xlab("Chemical") +
  ggtitle("BST ADDs for NSSS 95% conc., LAU scenarios") +
  theme_minimal() +
  theme(axis.text.x = element_blank(),
        panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank())
...
```{r}
p95_add_wide <-p95_add %>% pivot_wider(id_cols = c(Climate, CAS,
                                     `Chemical Name`,
                                     Receptor,
                                     Pathway,
                                     `Health Benchmark`),
...
pairs(formula = ~ Crop + Pasture + Reclamation,
      ...
      lower.panel = NULL,
      log = "xy",
      main = "Adult ADDs for NSSS 95% conc.")
```
...
```{r}
rank_crop <- p95_add_adult_crop %>%
  arrange(-Value) %>%
dplyr::select(`Chemical Name`, Value) %>%
  dplyr::rename(Crop = Value) %>%
  dplyr::mutate(Crop_Rank = 1:nrow(p95_add_adult_crop))

rank_rec <- p95_add_adult_rec %>%
  arrange(-Value) %>%
dplyr::select(`Chemical Name`, Value) %>%
  dplyr::rename(Reclamation = Value)  %>%
  dplyr::mutate(Reclamation_Rank = 1:nrow(p95_add_adult_rec))

rank_pasture <- p95_add_adult_pasture %>%
  arrange(-Value) %>%
dplyr::select(`Chemical Name`, Value) %>%
  dplyr::rename(Pasture = Value)  %>%
  dplyr::mutate(Pasture_Rank = 1:1:nrow(p95_add_adult_pasture))
...
rank_p95 <- rank_all %>%
  ...
  dplyr::mutate(Crop = paste0(sprintf("%.3g",
                ...
                Reclamation = paste0(sprintf("%.3g",
                              ...
                              Reclamation_Rank,
                              ")")) %>%
  write.csv("data/ADD_rank_p95.csv")
...
Importantly, predicted exposure does *not* imply predicted risk. In order to estimate potential risk, predicted exposures need to be compared to toxicity values (*e.g.*, reference doses).  

## Exposures from 1 ppb biosolids concentrations

Setting the biosolids concentration to 1 ppb for all chemicals allows a relative ranking of exposure potential.

```{r}
ppb_out <- readxl::read_excel("data/BST_results_LAU_ppb.XLSX",
                               sheet = "Human_Results")
```
...
ggplot(ppb_add) +
  ...
  scale_color_brewer(palette = "Set2") +
  xlab("BST-predicted total ingestion ADD (adult farmer), mg/kg/day") +
  ggtitle("Exposures from 1 ppb biosolids concentrations") +
  theme_bw() +
  theme(legend.position = c(0.8,0.8),
        legend.text = element_text(size = 12),
        legend.title = element_blank(),
...
ggplot(ppb_add_adult) +
  geom_point(aes(x = `Chemical Name`,
                 y = Value,
                 color = Scenario)) +
  scale_x_discrete(limits = ppb_add_adult_crop %>% pull(Chemical_fct)) +
  scale_y_log10(guide = "axis_logticks") +
  ylab("ADD, mg/kg/day") +
  xlab("Chemical") +
  ggtitle("BST ADDs for 1 ppb conc., LAU scenarios") +
  theme_minimal() +
  theme(axis.text.x = element_blank(),
        panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank())
...
```{r}
ppb_add_wide <-ppb_add %>% pivot_wider(id_cols = c(Climate, CAS,
                                     `Chemical Name`,
                                     Receptor,
                                     Pathway,
                                     `Health Benchmark`),
...
pairs(formula = ~ Crop + Pasture + Reclamation,
      ...
      lower.panel = NULL,
      log = "xy",
      main = "Adult ADDs for 1 ppb conc.")
```
...
```{r}
rank_crop <- ppb_add_adult_crop %>%
  arrange(-Value) %>%
dplyr::select(`Chemical Name`, Value) %>%
  dplyr::rename(Crop = Value) %>%
  dplyr::mutate(Crop_Rank = 1:nrow(ppb_add_adult_crop))

rank_rec <- ppb_add_adult_rec %>%
  arrange(-Value) %>%
dplyr::select(`Chemical Name`, Value) %>%
  dplyr::rename(Reclamation = Value)  %>%
  dplyr::mutate(Reclamation_Rank = 1:nrow(ppb_add_adult_rec))

rank_pasture <- ppb_add_adult_pasture %>%
  arrange(-Value) %>%
dplyr::select(`Chemical Name`, Value) %>%
  dplyr::rename(Pasture = Value)  %>%
  dplyr::mutate(Pasture_Rank = 1:1:nrow(ppb_add_adult_pasture))
...
rank_ppb <- rank_all %>%
  ...
  dplyr::mutate(Crop = paste0(sprintf("%.3g",
                ...
                Reclamation = paste0(sprintf("%.3g",
                              ...
                              Reclamation_Rank,
                              ")")) %>%
  write.csv("data/ADD_rank_ppb.csv")
